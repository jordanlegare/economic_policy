...
inline double lower_cvar(std::vector<double> values, double alpha) {
  if (values.empty()) return 0.0;
  std::sort(values.begin(), values.end());
  const std::size_t count = std::max<std::size_t>(1, static_cast<std::size_t>(std::ceil(clamp(alpha, 0.001, 1.0) * values.size())));
...
inline RobustRecommendationAnalysis analyze_robust_recommendations(
  ...
  std::vector<std::vector<double>> ca_surplus(package_count), us_surplus(package_count), values(package_count);
  std::vector<std::size_t> win_count(package_count, 0), ca_clear_count(package_count, 0),
      us_clear_count(package_count, 0), joint_clear_count(package_count, 0);
  for (std::size_t p = 0; p < package_count; ++p) {
    ca_surplus[p].reserve(sample_count); us_surplus[p].reserve(sample_count); values[p].reserve(sample_count);
  }

  std::mt19937_64 rng(seed);
  std::normal_distribution<double> normal(0.0, 1.0);
  for (std::size_t draw = 0; draw < sample_count; ++draw) {
    const double z_trade = normal(rng), z_border = normal(rng), z_pass = normal(rng);
    const double ca_g = clamp(canada_growth.mean + canada_growth.standard_deviation * normal(rng), canada_growth.lower_bound, canada_growth.upper_bound);
    const double us_g = clamp(us_growth.mean + us_growth.standard_deviation * normal(rng), us_growth.lower_bound, us_growth.upper_bound);
    const double ca_pi = clamp(canada_inflation.mean + canada_inflation.standard_deviation * normal(rng), canada_inflation.lower_bound, canada_inflation.upper_bound);
    const double us_pi = clamp(us_inflation.mean + us_inflation.standard_deviation * normal(rng), us_inflation.lower_bound, us_inflation.upper_bound);
    const double reservation_z = clamp(reservation_noise.standard_deviation * normal(rng), reservation_noise.lower_bound, reservation_noise.upper_bound);
    const double drawn_trade_elasticity = clamp(trade_elasticity.mean + trade_elasticity.standard_deviation * z_trade,
        trade_elasticity.lower_bound, trade_elasticity.upper_bound);
    const double drawn_border = clamp(border_friction.mean + border_friction.standard_deviation * z_border,
        border_friction.lower_bound, border_friction.upper_bound);
    const double drawn_pass = clamp(pass_through.mean + pass_through.standard_deviation * z_pass,
        pass_through.lower_bound, pass_through.upper_bound);

    std::vector<double> draw_values(package_count, -std::numeric_limits<double>::infinity());
    for (std::size_t p = 0; p < package_count; ++p) {
      const auto& package = negotiation.frontier[p];
      const Scenario* base = find_scenario(result, package.strategy_id);
      if (!base) {
        ca_surplus[p].push_back(-100.0); us_surplus[p].push_back(-100.0); values[p].push_back(-125.0);
        draw_values[p] = -125.0;
        continue;
      }

      Economy draw_economy = economy;
      draw_economy.trade_elasticity = drawn_trade_elasticity;
      draw_economy.border_friction = drawn_border;
      Scenario scenario = *base;
      scenario.growth += ca_g;
      scenario.us_growth += us_g;
      scenario.inflation += ca_pi;
      scenario.boc_score = clamp(scenario.boc_score + 1.4 * ca_g - 1.15 * ca_pi, 0.01, 100.0);
      scenario.federal_score = clamp(scenario.federal_score + 1.8 * ca_g - 0.85 * ca_pi, 0.01, 100.0);
      scenario.us_score = clamp(scenario.us_score + 1.6 * us_g - 0.90 * us_pi, 0.01, 100.0);

      const auto terms = package_terms(package);
      const auto evaluated = negotiation_detail::evaluate_terms(draw_economy, scenario, terms);
      const double residual_us_tariff = draw_economy.us_tariff_canada * (1.0 - terms.us_tariff_relief)
          * clamp(draw_economy.exports_to_us_share / 100.0, 0.0, 1.0);
      const double residual_ca_tariff = draw_economy.canada_retaliatory_tariff * (1.0 - terms.canada_tariff_relief)
          * clamp(draw_economy.imports_from_us_share / 100.0, 0.0, 1.0);
      const double friction_delta = drawn_border - border_friction.mean;
      const double canada_utility = clamp(evaluated.canada_utility
          - 0.025 * drawn_pass * residual_us_tariff - 0.20 * friction_delta, 0.0, 100.0);
      const double us_utility = clamp(evaluated.us_utility
          - 0.025 * drawn_pass * residual_ca_tariff - 0.16 * friction_delta, 0.0, 100.0);

      const double ca_reservation = clamp(negotiation.canada_reservation
          + 0.28 * ca_g - 0.20 * ca_pi + reservation_z, 0.0, 99.9);
      const double us_reservation = clamp(negotiation.us_reservation
          + 0.28 * us_g - 0.18 * us_pi + reservation_z, 0.0, 99.9);
      const double ca_s = canada_utility - ca_reservation;
      const double us_s = us_utility - us_reservation;
      const double value = decision_value(ca_s, us_s, economy.canada_priority, economy.us_priority);
      ca_surplus[p].push_back(ca_s); us_surplus[p].push_back(us_s); values[p].push_back(value);
      draw_values[p] = value;
      if (ca_s >= 0.0) ++ca_clear_count[p];
      if (us_s >= 0.0) ++us_clear_count[p];
      if (ca_s >= 0.0 && us_s >= 0.0) ++joint_clear_count[p];
    }
    const auto best = static_cast<std::size_t>(std::distance(draw_values.begin(),
        std::max_element(draw_values.begin(), draw_values.end())));
    if (best < win_count.size()) ++win_count[best];
  }

  std::vector<double> best_by_draw(sample_count, -std::numeric_limits<double>::infinity());
  for (std::size_t draw = 0; draw < sample_count; ++draw)
    for (std::size_t p = 0; p < package_count; ++p)
      best_by_draw[draw] = std::max(best_by_draw[draw], values[p][draw]);
  ...
  for (std::size_t p = 0; p < package_count; ++p) {
    ...
    metrics.canada_mean_surplus = mean(ca_surplus[p]);
    metrics.us_mean_surplus = mean(us_surplus[p]);
    metrics.canada_median_surplus = quantile(ca_surplus[p], 0.50);
    metrics.us_median_surplus = quantile(us_surplus[p], 0.50);
    metrics.canada_ci95_low = quantile(ca_surplus[p], 0.025);
    metrics.canada_ci95_high = quantile(ca_surplus[p], 0.975);
    metrics.us_ci95_low = quantile(us_surplus[p], 0.025);
    metrics.us_ci95_high = quantile(us_surplus[p], 0.975);
    metrics.canada_cvar10_surplus = lower_cvar(ca_surplus[p], analysis.cvar_tail_probability);
    metrics.us_cvar10_surplus = lower_cvar(us_surplus[p], analysis.cvar_tail_probability);
    metrics.canada_clear_probability = static_cast<double>(ca_clear_count[p]) / sample_count;
    metrics.us_clear_probability = static_cast<double>(us_clear_count[p]) / sample_count;
    metrics.joint_clear_probability = static_cast<double>(joint_clear_count[p]) / sample_count;
    ...
    metrics.clears_probability_gate = metrics.joint_clear_probability + 1e-12 >= analysis.required_joint_clear_probability;

    std::vector<double> regret(sample_count);
    for (std::size_t draw = 0; draw < sample_count; ++draw)
      regret[draw] = std::max(0.0, best_by_draw[draw] - values[p][draw]);
