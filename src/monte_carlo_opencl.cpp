...
class Runtime {
 public:
  ...
  bool run(const Input& input, const std::vector<Innovation>& innovations,
    ...
    scalars[2] = input.policy_rate; scalars[3] = input.core_inflation;
    scalars[4] = input.output_gap; scalars[5] = input.unemployment;
    scalars[6] = input.federal_debt_gdp; scalars[7] = input.housing_gap;
    scalars[8] = input.us_growth; scalars[9] = input.gdp_growth;
    scalars[10] = input.population_growth; scalars[11] = input.credit_spread;
    scalars[12] = input.fiscal_balance_gdp; scalars[13] = input.wage_growth;
    scalars[14] = input.headline_inflation; scalars[15] = input.global_growth;
    scalars[16] = input.inflation_expectations; scalars[17] = input.oil_price;
    scalars[18] = input.border_friction; scalars[19] = input.fx_pressure;
    scalars[20] = input.parameters.neutral_rate; scalars[21] = input.parameters.inflation_target;
    scalars[22] = input.parameters.rate_inflation_response; scalars[23] = input.parameters.rate_output_response;
    scalars[24] = input.parameters.max_quarterly_rate_step; scalars[25] = input.parameters.output_persistence;
    scalars[26] = input.parameters.fiscal_demand_multiplier; scalars[27] = input.parameters.real_rate_demand_sensitivity;
    scalars[28] = input.parameters.global_growth_sensitivity; scalars[29] = input.parameters.inflation_persistence;
    scalars[30] = input.parameters.inflation_expectations_weight; scalars[31] = input.parameters.phillips_curve_slope;
    scalars[32] = input.parameters.import_price_pass_through; scalars[33] = input.parameters.oil_inflation_sensitivity;
    scalars[34] = input.parameters.output_shock_sd; scalars[35] = input.parameters.inflation_shock_sd;
    scalars[36] = input.parameters.growth_shock_sd; scalars[37] = input.parameters.us_growth_shock_sd;
    scalars[38] = input.parameters.export_shock_sd; scalars[39] = input.parameters.us_export_shock_sd;

    std::array<double, kPathSeriesCount * kQuarterCount> paths{};
    auto copy_path = [&](std::size_t series, const auto& values) {
      std::copy(values.begin(), values.end(), paths.begin() + static_cast<std::ptrdiff_t>(series * kQuarterCount));
    };
 ...
 private:
  ...
  bool select_device(cl_device_type type) {
    ...
    for (const auto platform : platforms) {
      ...
      if (functions_.get_device_ids(platform, type, 0, nullptr, &device_count) != CL_SUCCESS || device_count == 0) continue;
  ...
  void ensure_probe() {
    if (probed_) return;
    probed_ = true;
    if (!load_library()) { probe_.detail = "OpenCL runtime library not available"; return; }
  ...
  std::string build_log() const {
    ...
    std::size_t bytes = 0;
    if (functions_.get_program_build_info(program_, device_, CL_PROGRAM_BUILD_LOG, 0, nullptr, &bytes) != CL_SUCCESS || bytes == 0) return {};
    std::string log(bytes, '\0');
    if (functions_.get_program_build_info(program_, device_, CL_PROGRAM_BUILD_LOG, bytes, log.data(), nullptr) != CL_SUCCESS) return {};
