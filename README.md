...
- **Decision layer:** separate BoC, Canadian federal/household and US loss functions combined into a preference-weighted Nash score with a fairness floor. Alongside twelve expert strategies, every request autonomously searches 144 monetary, fiscal, productive-investment, relief and diversification combinations for a customized win-win frontier. At startup, a U.S.-first compound comparison automatically prepares Mr. Greer's opening metrics while preserving a Canadian viability floor and calibrates the four deal controls; later tariff, delegation or sector changes produce a fresh, explicitly applicable recommendation without overwriting the user's choices.
