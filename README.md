...
- **Tariff fiscal ledger:** annual U.S. and Canadian customs revenue in both USD and CAD, calculated from the effective sector-weighted tariff and an elasticity-adjusted bilateral goods base. The dashboard also reports each country's mirrored bilateral balance, the remaining dollar gap, and progress toward the only outcome where both bilateral deficits are zero: balanced trade.
...
- **Bounded win-win allocation:** the Canada and U.S. outcome sliders are complementary shares of one agreement. Moving either slider immediately gives the remainder to the other party, so their displayed and modeled weights always total exactly 100%. Every evaluation also jointly searches the two countries' sector coverage, normalizes each side's deal value, and refreshes both countries' 0–100 deal scores as inflation, employment, risk, trade, or policy metrics change.
- **Adaptive bilateral growth floor:** every strategy projects separate Canadian and U.S. GDP-growth paths for all 12 quarters. The opening search tests guardrails from 0% through 2% in 0.1-point increments and admits a deal only when both countries clear the selected floor in every quarter; the ranking also includes output, employment and consumer-price results for every one of the 20 NAICS sectors.
- **Risk block:** recession frequency plus 90th-percentile inflation and federal debt stress outcomes.
- **Decision layer:** separate BoC, Canadian federal/household and US loss functions combined into a preference-weighted Nash score with a fairness floor. Alongside thirteen expert strategies, every request autonomously searches 288 monetary, fiscal, productive-investment, relief and diversification combinations for a customized win-win frontier. At startup, the engine compares all 101 whole-percentage bilateral allocations across every displayed strategy, selects the highest fairness-protected score, calibrates the four deal controls, and jointly tests every whole-percentage Canada–U.S. coverage pair in each sector. The browser applies that balanced sector equilibrium before presenting the first result and reruns the bilateral sector search after every change, atomically moving both rooms to the newly modeled win-win coverage.
- **Zero-deficit objective:** the search explicitly tests full rebalancing in every generated policy mix and includes a dedicated growth compact. It closes the shared gap with equal contributions from additional U.S. exports to Canada and Canadian exports redirected to other markets, strongly rewards the zero point, and keeps the adaptively selected bilateral growth floor binding. Since one country's bilateral surplus is the other's deficit after currency conversion, the shared absolute balance is the relevant win-win target rather than two independently manipulable deficit measures.
