...
- **Live bilateral negotiation rooms:** persistent header tabs open dedicated Canada/LeBlanc and USA/Greer workspaces where each party can set its headline position and tariff coverage across all 20 sectors. Changes are published through the shared AJAX negotiation endpoint; every open dashboard polls that session, applies newer revisions, reruns the full economy, and identifies the delegation behind the latest update.
...
- **Decision layer:** separate BoC, Canadian federal/household and US loss functions combined into a preference-weighted Nash score with a fairness floor. Alongside twelve expert strategies, every request autonomously searches 144 monetary, fiscal, productive-investment, relief and diversification combinations for a customized win-win frontier. At startup, a preference-neutral compound comparison automatically calibrates the four win-win controls; later tariff, delegation or sector changes produce a fresh, explicitly applicable recommendation without overwriting the user's choices.
