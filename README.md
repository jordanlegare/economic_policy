...
- **Continuous bilateral growth floor:** every strategy projects separate Canadian and U.S. GDP-growth paths for all 12 quarters, with modeled growth floored at 2% for both nations in every quarter. The bilateral guardrail verifies that the floor holds before an agreement can lead the ranking.
...
- **Zero-deficit objective:** the search explicitly tests full rebalancing in every generated policy mix and includes a dedicated growth compact. It closes the shared gap with equal contributions from additional U.S. exports to Canada and Canadian exports redirected to other markets, strongly rewards the zero point, and keeps the bilateral 2% growth floor binding. Since one country's bilateral surplus is the other's deficit after currency conversion, the shared absolute balance is the relevant win-win target rather than two independently manipulable deficit measures.
