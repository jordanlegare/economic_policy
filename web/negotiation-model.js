(() => {
  ...
  function inject() {
    ...
    section.innerHTML = `<div class="negotiation-head"><div><div class="eyebrow">Computational negotiation support</div><h2>BATNA · linked concessions · Pareto frontier · stability</h2><p>The macro engine generates country-specific payoffs; the bargaining layer searches linked packages, rejects deals below either side's reservation value, removes dominated packages, and tests unilateral incentives to defect.</p></div><div class="negotiation-engine-badge">VERIFIED PACKAGE SEARCH</div></div>
  ...
  function renderTrust(model, package_) {
    ...
    const checks = [
      ...
      ['20-sector schedule stochastic re-simulated', trust.sectorScheduleVerified === true],
      ['Clears both reservation values', package_.individuallyRational === true],
      ['Non-dominated within searched set', package_.paretoEfficient === true]
    ];
    const passed = checks.filter(x => x[1]).length;
    const integrity = trust.dataIntegrityPass === true && package_.verifiedWinWin === true;
    document.querySelector('#negotiationTrust').innerHTML = `<b>${integrity?'✓ VERIFIED WIN-WIN DATA PATH':'⚠ MODEL TRUST CHECK INCOMPLETE'}</b> · ${passed}/${checks.length} integrity checks pass · ${Number(trust.verificationMonteCarloDraws||0).toLocaleString()} verification draws · ${model.bargainingGridLevels||0} bargaining levels per linked issue.<br><span>${checks.map(x=>`${x[1]?'✓':'✕'} ${esc(x[0])}`).join(' · ')}</span>`;
  }
  ...
  function appendBriefing() {
    setTimeout(() => {
      ...
      const model = result.negotiation, package_ = model.recommendedPackage, trust = model.trust || {};
      if (!package_) return;
      ...
      const trustLine = trust.dataIntegrityPass && package_.verifiedWinWin
        ? `Verified data path: independent country trade channels; fixed mandate weights; trade balance excluded from welfare; sector schedule re-simulated with ${Number(trust.verificationMonteCarloDraws||0).toLocaleString()} common-random-number draws.`
      ...
      const section = `<section id="bargainingBriefSection"><h2>Bargaining analysis</h2>
        ...
        <p><b>Recommended Pareto package:</b> ${esc(package_.strategyName)} · Canada utility ${fmt(package_.canadaUtility,1)} (${signed(package_.canadaSurplus)} over reservation) · U.S. utility ${fmt(package_.usUtility,1)} (${signed(package_.usSurplus)} over reservation) · generalized Nash gain ${fmt(package_.nashGain,1)}.</p>
