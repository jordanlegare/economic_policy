...
function packageIdentityKey(s){return String(s?.id||'').startsWith('custom-')?`custom|${generatedPackageTitle(s)}`:`strategy|${String(s?.id||'')}|${String(s?.name||'')}`}
function bestUniqueScenarios(scenarios,limit=10){const seen=new Set(),unique=[];for(const s of scenarios||[]){const key=packageIdentityKey(s);if(seen.has(key))continue;seen.add(key);unique.push(s);if(unique.length>=limit)break}return unique}
...
window.PackageTitles={generatedPackageTitle,packageIdentityKey,bestUniqueScenarios,apply:applyMeaningfulPackageNames};
...
async function loadBaseline(){try{const [b]=await Promise.all([fetch('/api/baseline').then(r=>r.json()),pollNegotiation(true)]);settings=b.settings;$('#dataStatus').textContent=b.status==='live-partial'?'Partial live official feeds':'Documented calibrated/default baseline';$('#asOf').textContent='As of '+new Date(b.asOf).toLocaleString();$('#sync').textContent=b.status==='live-partial'?'Partial official-feed refresh':'Baseline fallback active';$('#sourceList').innerHTML=b.sources.map(s=>`<a href="${s.url}" target="_blank" rel="noreferrer"><b>${s.name}</b><span>${s.fields} ↗</span></a>`).join('');}catch(e){settings={};$('#dataStatus').textContent='Baseline unavailable';$('#sync').textContent='Connection error';}evaluate();setInterval(()=>pollNegotiation(false),1000)}
