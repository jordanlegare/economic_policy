(() => {
  ...
  function inject() {
    ...
    section.innerHTML = `
      ...
      <div id="robustPreviewNote" class="room-warning" hidden></div>
      <div class="room-grid">
  ...
  function renderRobustness() {
    ...
    const recommended=r.recommendedPackage || r.packages?.find(p=>p.packageId===r.recommendedPackageId);
    ...
    const note=document.querySelector('#robustPreviewNote');
    const robustTotal=Number(r.packageTotal ?? r.packages?.length ?? 0);
    const robustShown=Number(r.packageReturned ?? r.packages?.length ?? 0);
    const negotiationTotal=Number(result.negotiation?.frontierTotal ?? result.negotiation?.frontier?.length ?? 0);
    const negotiationShown=Number(result.negotiation?.frontierReturned ?? result.negotiation?.frontier?.length ?? 0);
    const truncated=Boolean(r.packageTruncated || result.negotiation?.frontierTruncated);
    if(note){note.hidden=!truncated;note.textContent=truncated?`Interactive preview only: ${robustShown} of ${robustTotal} robust package rows and ${negotiationShown} of ${negotiationTotal} negotiation packages are materialized in the browser. The complete frontier was evaluated and remains in the server-side session.`:'';}
