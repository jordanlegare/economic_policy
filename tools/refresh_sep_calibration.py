...
def estimate(rows):
    ...
    E=[R("output_persistence",bg[0],seg[0],max(.05,bg[0]-1.96*seg[0]),min(.98,bg[0]+1.96*seg[0]),len(yg),"OLS production-form output-gap equation",.05<bg[0]<.98,"Real-time SEP vintages"),R("real_rate_demand_sensitivity",bg[1],seg[1],max(.001,bg[1]-1.96*seg[1]),max(.002,bg[1]+1.96*seg[1]),len(yg),"OLS production-form output-gap equation",bg[1]>0,"Coefficient on -(staff rate-neutral rate)"),R("output_shock_sd",sdg,0,max(.001,.7*sdg),1.3*sdg,len(yg),"residual SD of output equation",False,"SEP current-quarter residual scale retained as reference-only; not treated as realized macro shock variance"),R("inflation_persistence",bp[0],sep[0],max(.05,bp[0]-1.96*sep[0]),min(.98,bp[0]+1.96*sep[0]),len(yp),"OLS production-form centered inflation equation",.05<bp[0]<.98,"PCPIX year-over-year within each vintage"),R("phillips_curve_slope",bp[1],sep[1],bp[1]-1.96*sep[1],bp[1]+1.96*sep[1],len(yp),"OLS production-form centered inflation equation",bp[1]-1.96*sep[1]>0,"Contemporaneous SEP output gap; direct promotion requires a positive 95% interval"),R("fx_pass_through",bp[2],sep[2],bp[2]-1.96*sep[2],bp[2]+1.96*sep[2],len(yp),"OLS production-form centered inflation equation",bp[2]>0,"Omitted oil/import/supply terms remain in residual"),R("inflation_shock_sd",sdp,0,max(.001,.7*sdp),1.3*sdp,len(yp),"residual SD of inflation equation",False,"SEP current-quarter residual scale retained as reference-only; not treated as realized macro shock variance"),R("rate_inflation_response",a,0,max(.01,.65*a),1.35*a,np,"grid fit exact clipped production policy rule",False,f"SEP implied-rate path RMSE={rmse:.4f} pp; staff path is not a Governing Council reaction function"),R("rate_output_response",b,0,max(.001,.65*b),max(.002,1.35*b),np,"grid fit exact clipped production policy rule",False,f"SEP implied-rate path RMSE={rmse:.4f} pp; boundary estimate retained as reference-only"),R("max_quarterly_rate_step",s,0,max(.025,.75*s),1.25*s,np,"grid fit exact clipped production policy rule",False,f"SEP implied-rate path RMSE={rmse:.4f} pp; staff path is not an observed policy-adjustment constraint")]
