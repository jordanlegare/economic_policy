...
def valet_series(series: str):
    url = BOC.format(series=series)
    raw = request_bytes(url)
    payload = json.loads(raw)
    points = []
    for observation in payload.get("observations", []):
        date = observation.get("d", "")
        cell = observation.get(series) or observation.get(series.upper()) or observation.get(series.lower())
...
def policy_grid(rows):
    # Fit the exact production adjustment rule at quarterly frequency.
    obs = []
    ...
    best = None
    # Coarse grid, then two local refinements.
    grids = [
        ([0.25 + .10 * i for i in range(23)], [0.0 + .05 * i for i in range(21)], [.125 + .025 * i for i in range(36)]),
    ]
    center = None
    for stage in range(3):
        if stage == 0:
            alphas, betas, steps = grids[0]
        else:
            a0, b0, s0 = center
...
def estimate(rows):
    ...
    # Output-gap equation: gap_t = rho gap_(t-1) - beta(rate_t-r*) + error.
    x_gap, y_gap, q_gap = [], [], []
    for row in rows:
        prev = byq.get(previous_quarter(row["quarter"]))
        if not prev:
    ...
    # Inflation equation in exactly the engine's centered sign convention:
    # pi_t-2 = rho(pi_(t-1)-2) + kappa*gap + fx*(USDCAD-1.34)
    #          - oil_coeff*(WTI-75) + error.
    x_pi, y_pi, q_pi = [], [], []
    for row in rows:
        prev = byq.get(previous_quarter(row["quarter"]))
        if not prev:
            continue
        x_pi.append([
            float(prev["core_inflation"]) - 2.0,
            float(row["output_gap"]),
            float(row["usdcad"]) - 1.34,
            -(float(row["wti"]) - 75.0),
        ])
    ...
    def rec(parameter, value, se, lo, hi, nobs, method, eligible, notes):
        return {
            ...
            "upper_bound": hi,
            "observations": nobs,
            "sample_start": rows[0]["quarter"],
            "sample_end": rows[-1]["quarter"],
            "method": method,
            "direct_eligible": "true" if eligible else "false",
            "notes": notes,
        }

    estimates = []
    estimates += [
        rec("output_persistence", b_gap[0], se_gap[0], max(.05, b_gap[0] - 1.96*se_gap[0]), min(.98, b_gap[0] + 1.96*se_gap[0]), len(y_gap), "OLS production-form output-gap equation", .05 < b_gap[0] < .98, "No intercept; policy wedge uses the official neutral-rate midpoint."),
        ...
        rec("output_shock_sd", sd_gap, 0.0, max(.001, .70*sd_gap), 1.30*sd_gap, len(y_gap), "sample SD of output-equation residual", sd_gap > 0, "Residual innovation scale; bounds are +/-30% sensitivity around fitted residual SD."),
        ...
        rec("oil_inflation_sensitivity", b_pi[3], se_pi[3], b_pi[3] - 1.96*se_pi[3], b_pi[3] + 1.96*se_pi[3], len(y_pi), "OLS production-form centered inflation equation", True, "Uses engine sign convention -coefficient*(WTI-75); signed estimate is allowed."),
        rec("inflation_shock_sd", sd_pi, 0.0, max(.001, .70*sd_pi), 1.30*sd_pi, len(y_pi), "sample SD of inflation-equation residual", sd_pi > 0, "Residual innovation scale; bounds are +/-30% sensitivity around fitted residual SD."),
...
def refresh(root: Path):
    collected = {}
    manifest = []
    modes = {"output_gap": "last", "cpi_trim": "last", "cpi_median": "last",
             "policy_rate": "last", "usdcad": "mean", "wti": "mean"}
    for field, series in SERIES.items():
        points, raw, url = valet_series(series)
        collected[field] = quarterly_aggregate(points, modes[field])
        manifest.append({"source_id": f"boc_valet_{field}", "agency": "Bank of Canada",
                         "dataset": series, "url": url, "sha256": sha256(raw),
                         "transformation": f"quarterly {modes[field]}"})
    ...
    manifest += [
        ...
        {"source_id": "statcan_unemployment_monthly", "agency": "Statistics Canada", "dataset": "14-10-0287-01", "url": STATCAN_LFS, "sha256": sha256(lfs_raw), "transformation": "quarterly mean of seasonally adjusted unemployment rate"},
    ]

    quarters = sorted(set.intersection(*(set(v) for v in collected.values())), key=quarter_key)
    panel = []
    for q in quarters:
        trim = collected["cpi_trim"][q]
        median = collected["cpi_median"][q]
        panel.append({
            "quarter": q,
            "output_gap": collected["output_gap"][q],
            "core_inflation": .5 * (trim + median),
            "cpi_trim": trim,
            "cpi_median": median,
            "policy_rate": collected["policy_rate"][q],
            "usdcad": collected["usdcad"][q],
            "wti": collected["wti"][q],
            "gdp_growth": collected["gdp_growth"][q],
            "unemployment": collected["unemployment"][q],
        })
    if len([r for r in panel if SAMPLE_START <= r["quarter"] <= SAMPLE_END]) < 60:
        spans = {k: (min(v, key=quarter_key), max(v, key=quarter_key), len(v)) for k, v in collected.items() if v}
        raise RuntimeError(f"Insufficient complete sample; spans={spans}")

    estimates, covariance = estimate(panel)
    data = root / "data" / "calibration"
    write_csv(data / "quarterly_estimation_panel.csv",
              ["quarter", "output_gap", "core_inflation", "cpi_trim", "cpi_median", "policy_rate", "usdcad", "wti", "gdp_growth", "unemployment"], panel)
    write_csv(data / "quarterly_estimation_manifest.csv",
              ["source_id", "agency", "dataset", "url", "sha256", "transformation"], manifest)
    write_csv(data / "quarterly_structural_estimates.csv",
              ["parameter", "estimate", "standard_error", "lower_bound", "upper_bound", "observations", "sample_start", "sample_end", "method", "direct_eligible", "notes"], estimates)
    write_csv(data / "quarterly_residual_covariance.csv",
              ["row", "column", "covariance", "correlation", "observations"], covariance)
    print(f"panel_rows={len(panel)} sample_rows={len([r for r in panel if SAMPLE_START <= r['quarter'] <= SAMPLE_END])}")
    for e in estimates:
...
def verify(root: Path):
    ...
    for row in expected:
        ...
        if not old:
            raise RuntimeError(f"Missing committed estimate {row['parameter']}")
        if abs(float(old["estimate"]) - row["estimate"]) > 5e-8:
            raise RuntimeError(f"Estimate drift {row['parameter']}: {old['estimate']} vs {row['estimate']}")
        if old["direct_eligible"] != row["direct_eligible"]:
            raise RuntimeError(f"Eligibility drift {row['parameter']}")
    with (data / "quarterly_residual_covariance.csv").open(newline="", encoding="utf-8") as f:
        committed_cov = list(csv.DictReader(f))
    if len(committed_cov) != 4:
        raise RuntimeError("Residual covariance must be 2x2")
    if abs(float(committed_cov[1]["correlation"]) - covariance[1]["correlation"]) > 5e-8:
        raise RuntimeError("Residual correlation drift")
